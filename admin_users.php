...
if (isset($_POST['tclass_save']))
{
    ...
    if ($tclass_start == 0 || ($tclass_end <= $tclass_start && $tclass_start > 0 && $tclass_end > 0) || ($tclass_start < 1 && $tclass_end < 1) || ($tclass_start > 0 && $tclass_start < time()) || ($tclass_end >
        0 && $tclass_end < time()))
    {
...
<form method='post' action='" . e_SELF . "' id='tclassform' name='tclassform' >
	...
	<table class='fborder' style='" . ADMIN_WIDTH . "'>
		...
		<tr>
			...
			<td class='forumheader3' ><input type='text' id='tclass_user' name='tclass_user' class='tbox' value='" . $user_name . "' />&nbsp;&nbsp;<img src='" . e_IMAGE . "/generic/" . IMODE .
        "/user_select.png'
			style='width:16px;height:16px;; vertical-align:top;border:0px;' alt='" . TCLASS_A43 . "'
			title='" . TCLASS_A43 . "' onclick=\"window.open('" . e_HANDLER .
        "user_select_class.php?text.tclass_user','user_search', 'toolbar=no,location=no,status=yes,scrollbars=yes,resizable=yes,width=300,height=200,left=100,top=100'); return false;\" /></td>
		</tr>";
    // calendar options
   ...
   $tclasssinput = e107::getForm()->datepicker($prune_cal_attrib['name'], $prune_cal_attrib['value']);
    $tclass_text .= "
		<tr>
...
", false))
    {
		...
		<tr>
			...
			<td class='forumheader3' ><img src='images/" . ($tclass_donestart == 1 ? "green" : "red") . ".png' /> &nbsp;" . ($tclass_start > 0 ? date(str_replace("-H-M", " H:i", $tclass_obj->
                tclass_dateform), $tclass_start) : "") . "</td>
